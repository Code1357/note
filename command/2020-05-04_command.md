...
# yarn    
- yarn（ヤーン）は、Node.js のためのパッケージマネージャ
- ライブラリのパッケージがインストールされているのかを記録し、新しいパッケージのインストールや削除を簡単に行えるようにするプログラム  
- Node.jsは標準で、npmというパッケージ管理ツールがあるが、yarnはnpmの範囲もカバーしている  
[npm_page](https://www.npmjs.com/)  
[yanr_ver確認](https://www.npmjs.com/package/yarn)
## install  
```
...
- 基本的にライブラリはローカルインストールで行う（現在のディレクトリのみインストールする）

ディレクトリの場所を決める
```  
cd ~/workspace/
git clone git@github.com:自分のGitHubアカウント名/yarn-training.git
cd yarn-training
```  
[axios](https://github.com/axios/axios)をローカルにインストールする
```  
yarn add axios
```  
ディレクトリ状況
![yarn_01](img/yarn_01.png)  
axiosを使えるようにする(app.jsを実装)
```js:spp.js  
'use strict';
